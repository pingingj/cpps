...
/*   Updated: 2026/06/02 18:22:02 by dgarcez-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	parse_btc(std::string &date, std::string &value, bool file)
{
	...
	if (file == DB && (value.find_first_not_of("1234567890.") != value.npos || std::count(value.begin(), value.end(), '.') > 1))
		return (false);
	if (file == INPUT && (value.find_first_not_of("1234567890. ") != value.npos || std::count(value.begin(), value.end(), '.') > 1))
	{
		std::cerr << "Error: Invalid value" << std::endl;
		return (false);
	}
	if (file == INPUT && (std::count(value.begin(), value.end(), ' ') != 1 || std::count(date.begin(), date.end(), ' ') != 1))
	{
	...
	if (file == DB && (date.find_first_not_of("1234567890-") != date.npos || std::count(date.begin(), date.end(), '-') > 2))
		return (false);
	if (file == INPUT && (date.find_first_not_of("1234567890- ") != date.npos || std::count(date.begin(), date.end(), '-') > 2))
	{
